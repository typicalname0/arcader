...
function file_publish(){
	...
	$w.title("Arcader Artist");
	$w.$main.text("ARE YOU SURE that this drawing will NOT break the terms of service and is NOT low effort?");
	$w.$Button("Yes (Publish)", () => {
		$w.close();
		// Publish to Arcader
		$.ajax({
			type: 'POST',
			...
			success: (r) => {
				const $w = new $FormToolWindow().addClass("dialogue-window");
				$w.title("Arcader Artist -- Response from server");
				$w.$main.text(r);
				$w.center();
			}
		})
	})[0].focus();
	$w.$Button("No (Return to drawing)", () => {
		$w.close();
	});
	$w.center();
}
